...
function DonationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isCustomAmount && (!customAmount || parseFloat(customAmount.replace(',', '.')) <= 0)) {
    ...
    try {
      ...
      setError(err.message || t('ContributePage.formSection.errors.unexpected'));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (success) {
    return (
      <div className="w-full bg-white p-6 rounded-lg shadow-md text-center">
        <div className="w-16 h-16 rounded-full bg-[#509f8c] flex items-center justify-center text-white text-3xl mx-auto mb-4">✓</div>
        <h2 className="text-2xl font-bold mb-4">{t('ContributePage.formSection.success.title')}</h2>
    ...
    <form onSubmit={handleSubmit} className="w-full bg-white">        
      <div className="grid grid-cols-3 md:grid-cols-5 gap-2 mb-6">
        {getDonationOptions(currency).map((option) => (
          ...
          >
            <div className={`font-bold ${ option.label.length > 5 ? 'text-xl' : 'text-2xl' }`}>{option.label}</div>
